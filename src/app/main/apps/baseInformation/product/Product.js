...
const schema = yup.object().shape({
  ...
  code: yup
    .string()
    .required('شما باید کد محصول را وارد کنید'),
  type: yup
    .number()
    .required('شما باید نوع محصول را انتخاب کنید'),
  unitId: yup
    .number()
    .required('شما باید واحد سنجش محصول را وارد کنید')
});
...
function Product(props) {
      ...
      if (productId === 'new') {
        ...
        dispatch(getProduct(routeParams)).then((action) => {
          ...
          if (!action.payload) {
            ...
            getSubUnits.push(action.payload.productSubUnits);
          }
        });
      }
    }
  ...
  useEffect(() => {
    if (!product || product.title === undefined) {
      return;
    }
    /**
  ...
  if (
    ...
    // routeParams.productId !== product.productId)
  ) {
    return <FuseLoading />;
  }
        ...
        content={
          <div className="p-16 sm:p-24 max-w-2xl">
            <div className={tabValue !== 0 ? 'hidden' : ''}>
              <BasicInfoTab subMeasureUnit={getSubUnits} />
            </div>
...
export default withReducer('baseInformation', reducer)(Product);
