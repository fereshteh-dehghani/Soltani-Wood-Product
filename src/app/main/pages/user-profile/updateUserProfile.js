...
const UpdateUserProfile = () => {
  ...
  //   const selectedBranch =resultBranch.filter(br => br.id ===result.branchId);
  //   return selectedBranch[0];
  // }
  // const valueBranch = selectBranchId();
  ...
  const changeHandle = (e) => {
    ...
    reader.onloadend = (evt) => {
      if (evt.target.readyState === FileReader.DONE) {
        ...
        array.map((x) => fileByteArray.push(x));
      }
      setUserImage(fileByteArray);
    };
  };
  useEffect(() => {
    try {
    ...
    try {
      axios
        .post(apiUrlUpdateUserProfile, {
          ...
          photoFileName:
            photoFileName === undefined || photoFileName === null
                    ...
                    resultBranch.map((branch) => {
                      return (
                        <MenuItem
                          key={branch.name}
                          value={branch.id}
                          selected={branch.id === result.branchId && 'selected'}
                        >
                          {branch.name}
                        </MenuItem>
                      );
